...
class Trainer(ABC):
    ...
    def __init__(
        ...
        scheduler: Optional[Union[torch.optim.lr_scheduler.LRScheduler, "core.SchedulerBase"]],
        *,
        log_bar: bool = True,
    ...
    def _to_device(
        ...
        if args:
            if len(args) == 1:
                return args[0].to(self.device[0]) if "to" in args[0].__dir__() else args[0]
            else:
                return tuple([a.to(self.device[0]) if "to" in a.__dir__() else a for a in args])
        elif kwargs:
            return {k: v.to(self.device[0]) if "to" in v.__dir__() else v for k, v in kwargs.items()}

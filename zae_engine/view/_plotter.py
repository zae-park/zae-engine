...
class Plotter:
    ...
    def draw_30s_graph(self, line):
        ...
        r_peaks = self.r_peaks[
            np.where((self.r_peaks >= line * self.fs * self.sec_in_line) &
                     (self.r_peaks < (line + 1) * self.fs * self.sec_in_line))[0]]
    ...
    def draw_graph(self):
        ...
        for i in range(self.lines_load, min(total_lines, self.lines_load + self.lines_add)):
            ...
            if len(self.r_peaks) > 0:
                # if self.vs_filtered:
                r_peak_range = np.where((np.array(self.r_peaks) >= i * self.fs * self.sec_in_line) &
                                        (np.array(self.r_peaks) < (i + 1) * self.fs * self.sec_in_line))[0]

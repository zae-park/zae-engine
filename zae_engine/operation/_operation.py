...
def sanity_check(
    onoff: Union[np.ndarray, torch.Tensor, list],
    ...
    if incomplete_only:
        return onoff.astype(int).tolist()
    ...
    for closed_on, closed_off, _ in close_interval_onoff:
        ...
        final_onoff = merge_and_remove(
            onoffs=onoff_set, interval=interval_set, merge_th=tol_merge, remove_th=tol_interval
        )

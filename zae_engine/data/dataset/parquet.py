...
class ParquetCategoryDataset(data.Dataset):
    ...
    def _cache_parquet(self, idx):
        ...
        for df, fpar in zip(fparquet.iter_row_groups(columns=self.raw_cols), fparquet):
            if fpar.cats:
                df["session_id"] = fpar.cats["session_id"][0]
            else:
                df["session_id"] = fpar.info["name"]
            list_df.append(df)
    ...
    def _transform_raw_to_array(self, pd_raw_data):
        ...
        # sample = self.row_canvas(**pd_raw_data.drop('session_id'))
        # sample.session_id = pd_raw_data.session_id
        return sample
        # dict_array_data = {}
        # if 'prod_id' in self.use_cols:
        #     dict_array_data['prod_id'] = np.array(int(pd_raw_data['unique_id']))
        # if 'output' in self.use_cols:
        #     dict_array_data['output'] = np.array(pd_raw_data['cat_id'])
        # if 'img_input' in self.use_cols:
        #     dict_array_data['img_input'] = self.jpg_array_to_img_matrix(pd_raw_data['img_jpg'])

    def jpg_array_to_img_matrix(self, path):
        return path

...
if __name__ == "__main__":
    ...
    # 함수 사용 예제
    print(example_func(np.array([1, 2, 3]), np.array([4, 5, 6]), np.array([7, 8, 9])))
    # 첫 두 개의 numpy 배열이 torch tensor로 변환됩니다.

    # 클래스 메소드 사용 예제
    print(example.example_method({"x": np.array([1, 2, 3]), "y": np.array([4, 5, 6]), "z": np.array([7, 8, 9])}))
    # 'x'와 'y' 키의 값이 torch tensor로 변환됩니다.

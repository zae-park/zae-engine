import unittest
import torch
from skconv import SKConv1D  # SKConv1D가 정의된 모듈의 이름이 skconv라고 가정합니다.
...
class TestSKConv1D(unittest.TestCase):
    ...
    def test_basic_functionality(self):
        model = SKConv1D(ch_in=self.ch_in, ch_out=5)
    ...
    def test_different_kernel_sizes(self):
        model = SKConv1D(ch_in=self.ch_in, ch_out=5, kernels=[3, 5, 7])
        input_tensor = torch.randn(self.batch_size, self.ch_in, self.seq_len)
        output = model(input_tensor)
        self.assertEqual(output.shape, (self.batch_size, 5, self.seq_len))
    ...
    def test_output_size(self):
        out_size = 16
        model = SKConv1D(ch_in=self.ch_in, ch_out=5, out_size=out_size)
        input_tensor = torch.randn(self.batch_size, self.ch_in, self.seq_len)
        output = model(input_tensor)
    ...
    def test_identity_out_pool(self):
        model = SKConv1D(ch_in=self.ch_in, ch_out=5, out_size=None)
        input_tensor = torch.randn(self.batch_size, self.ch_in, self.seq_len)
        output = model(input_tensor)
        self.assertEqual(output.shape, (self.batch_size, 5, self.seq_len))
    ...
    def test_invalid_kernel_size(self):
        with self.assertRaises(AssertionError):
            SKConv1D(ch_in=self.ch_in, ch_out=5, kernels=[2, 4])  # Even kernel sizes should raise an assertion error.

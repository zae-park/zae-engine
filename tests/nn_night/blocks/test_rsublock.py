...
class TestRSUBlock(unittest.TestCase):
    def setUp(self):
        # unet_block.RSUBlock 초기화: in_ch=3, mid_ch=32, out_ch=3, height=7, dilation_height=2
        self.rsu_block = unet_block.RSUBlock(in_ch=3, mid_ch=32, out_ch=3, height=7, dilation_height=2)
    ...
    def test_dilation_height(self):
        ...
        rsu_block_dilate = unet_block.RSUBlock(in_ch=3, mid_ch=32, out_ch=3, height=7, dilation_height=3)
        input_tensor = torch.randn(1, 3, 256, 256)
    ...
    def test_height_variation(self):
        ...
        rsu4 = unet_block.RSUBlock(in_ch=3, mid_ch=32, out_ch=3, height=4, dilation_height=2)
        input_tensor = torch.randn(1, 3, 256, 256)
    ...
    def test_channel_change(self):
        # unet_block.RSUBlock을 다른 채널로 초기화 (in_ch=3, mid_ch=64, out_ch=6)
        rsu_block_channel = unet_block.RSUBlock(in_ch=3, mid_ch=64, out_ch=6, height=7, dilation_height=2)
        input_tensor = torch.randn(1, 3, 256, 256)

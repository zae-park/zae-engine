...
class TestNestedUNet(unittest.TestCase):
    ...
    def test_forward_pass_different_input_sizes(self):
        ...
        input_sizes = [
            (1, self.in_ch, 128, 128),
            (2, self.in_ch, 256, 256),
            (4, self.in_ch, 512, 512)
        ]
        for size in input_sizes:
            with self.subTest(size=size):
                ...
                try:
                    output, *side_outputs = self.model(x)
                    self.assertEqual(output.shape, (size[0], self.out_ch, size[2], size[3]),
                                     f"Final output shape mismatch for input size {size}.")
                    for i, side in enumerate(side_outputs):
                        self.assertEqual(side.shape, (size[0], self.out_ch, size[2], size[3]),
    ...
    def test_forward_pass_custom_heights_and_dilations(self):
        ...
        try:
            ...
            self.assertEqual(output.shape, (1, self.out_ch, 256, 256), "Final output shape mismatch with custom heights and dilations.")
            for i, side in enumerate(side_outputs):
                self.assertEqual(side.shape, (1, self.out_ch, 256, 256), f"Side output {i+1} shape mismatch with custom heights and dilations.")
        except Exception as e:
    ...
    def test_forward_pass_different_channels(self):
        ...
        model = NestedUNet(
            ...
            width=16,
            heights=[6, 5, 4, 3],
            dilation_heights=[1, 2, 3, 4]
        )
        model.eval()
        x = torch.randn(1, in_ch, 256, 256)
        try:
            output, *side_outputs = model(x)
            self.assertEqual(output.shape, (1, out_ch, 256, 256), "Final output shape mismatch with different channels.")
            for i, side in enumerate(side_outputs):
                self.assertEqual(side.shape, (1, out_ch, 256, 256), f"Side output {i+1} shape mismatch with different channels.")
        except Exception as e:
    ...
    def test_side_outputs_count(self):
        ...
        x = torch.randn(1, self.in_ch, 256, 256)
        output, *side_outputs = self.model(x)
        self.assertEqual(len(side_outputs), self.model.num_layers, "Number of side outputs does not match number of layers.")
    ...
    def test_forward_pass_gpu(self):
        ...
        if torch.cuda.is_available():
            ...
            try:
                ...
                for i, side in enumerate(side_outputs):
                    self.assertEqual(side.shape, (1, self.out_ch, 256, 256), f"Side output {i+1} shape mismatch on GPU.")
            except Exception as e:
